...
| 3단 해설 | 논문마다 요약(3분) → 비유로 이해하기 → 쉽게 풀어 읽기(전체 흐름 5~10분, 원문 대비 30% 분량 재서술) |
| 읽기 진행 | 카드의 "다 읽음" 토글 → 별자리에서 읽은 별은 금빛, 다음 별은 반짝임, "이어 읽기" 버튼 |
| 논문 구조도 | 논문마다 직접 그린 SVG 다이어그램 |
| 별자리 탐색 지도 | 19편을 밤하늘 별자리로 배치 — 별을 누르면 해당 논문으로 이동 |
| 용어사전 359개 | 난이도 별점(★~★★★★★), 핵심 개념 24개 형광 표시, 검색·난이도 필터, 벤치마크 용어 포함 |
